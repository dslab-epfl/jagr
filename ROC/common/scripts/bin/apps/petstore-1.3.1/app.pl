...
sub init_app {
    ...
    $PETSTORE_HOME = "$PPAPPDIR/petstore";
...
sub get_app_loadgen_checksum() {
    ...
    my $tracesdir=$rocdir . "/common/scripts/traces/petstore-1.3.1/";
...
sub get_app_fault_trigger_dir() {
    return $PPROOTDIR . "/expts/faults/petstore-1.3.1-beans/";
}

sub get_app_root_buggy_src_dir() {
    return $PPAPPDIR . "/petstore-1.3.1-buggysrc/deployment-files/";
}

sub get_app_buggy_src_dirs() {
    my $BUGGY_ROOT_DIR=$PPAPPDIR . "/petstore-1.3.1-buggysrc/deployment-files/";
    
#    print "petstore-1.3.1/app.pl:: buggy root dir is '" . $BUGGY_ROOT_DIR . "'\n";
