...
public class CheckSumHelper {
	...
	new TextMunger() {
		...
		public String munge( Engine engine, HttpRequest req, HttpResponse resp,
		    ...
		    engine.logDebug( "ps13munger: munged " + debugcount + " sets of form tags" );
	...
	new TextMunger() {
		...
		public String munge( Engine engine, HttpRequest req, HttpResponse resp,
		    ...
		    engine.logDebug( "ps13munger: running on request: " + req.url.getFile() );


		    boolean munge = false;
		    for( int i=0; i<mungeableURL.length; i++ ) {
			if( req.url.getFile().endsWith( mungeableURL[i] )) {
			    engine.logDebug( "ps13munger: matches " + mungeableURL[i] );
			    munge = true;
			}
		    }

		    if( !munge) {
			engine.logDebug( "ps13munger: doesn't any mungeable URLs" );
			return buf;
		    }
    ...
    CheckSumHelper() throws AbortSessionException {
        try {
	...
	engine.logDebug( "calculateCheckSum: considering running mungers on http response" );
        
        if(resp.contentType != null && resp.contentType.startsWith("text")) {
            String strbuf = new String(buf);
	    
	    engine.logDebug( "calculateCheckSum: about to run mungers on http response" );
