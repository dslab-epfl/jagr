 ...
 *
 */
public class RecoMgrConnector implements Plugin, RecordCollectionListener {

    static Logger log = Logger.getLogger( "RecoMgrConnector" );
	...
	while( iter.hasNext() ) {
		...
		name = (String)((Identifiable)val).getId().get( "name" );
                ipaddr = (String)((Identifiable)val).getId().get( "ipaddress" );
                //
                // you can also pull out the "hostname" if that's preferable.
                //

		if( name == null ) {
		    log.error( "ACK! 'name' of component is null.  fix this in RecoMgrConnector!!!!" );
		}
	    }
	    else {
	    ...
	    report.addSuspect( new FailureReport.Suspect( name, /*ipaddr,*/ ro.getRank() ));
	}

	sendFailureReport( report );
    }

    private void sendFailureReport( FailureReport report ) {
	try {
	    ...
	    log.info( "Sending Failure Report [time=" + new Date() + "]: " +
				report );
	    socket.send( packet );
