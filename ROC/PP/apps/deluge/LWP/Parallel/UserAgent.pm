# -*- perl -*-
# $Id: UserAgent.pm,v 1.2 2003/03/14 12:15:44 emrek Exp $
...
an C<LWP::Parallel::UserAgent::Entry> object.
...
sub _connect {
  ...
  my ($error_response, $proxy, $protocol, $timeout, $use_eval) = 
    $self->init_request ($request, $entry);
  if ($error_response) {
...
sub handle_response
{
	...
	$entry->cookie_jar->extract_cookies($response)
	    if $entry->cookie_jar;
...
sub init_request {
    ...
    my ($agent, $from, $timeout, $use_eval, $parse_head, $max_size) =
		@{$self}{qw(agent from timeout use_eval parse_head max_size)};

    # Set User-Agent and From headers if they are defined
	if ($entry->agent) {
		$request->header('User-Agent' => $entry->agent);
	} elsif ($agent) {
		$request->header('User-Agent' => $agent);
	}
    $request->header('From' => $from) if $from;
    $request->header('Range' => "bytes=0-$max_size") if $max_size;
    $entry->cookie_jar->add_cookie_header($request) if $entry->cookie_jar;
