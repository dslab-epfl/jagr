...
public class LargeOrderBmpEJB extends LargeOrderCmpEJB {
    ...
    private int                  salesOrderIdCache;
    private int                  orderLineNumberCache;
    ...
    public Integer ejbCreate(
        ...
        super.ejbCreate(salesOrderId, orderLineNumber, assemblyId, qty, dueDate);
                        
       idCache = id;
       salesOrderIdCache = salesOrderId;
       orderLineNumberCache = orderLineNumber;
        ...
        try {
            ...
            stmt = conn
                .prepareStatement("DELETE FROM M_largeorder WHERE lo_id = ? " +
                                "AND lo_o_id = ? AND lo_ol_id = ? AND lo_assembly_id = ? " +
                                "AND lo_qty = ? AND lo_due_date = ? ");

            stmt.setInt(1, id.intValue());

            stmt.setInt(2, salesOrderIdCache);
            stmt.setInt(3, orderLineNumberCache);
            stmt.setString(4, assemblyIdCache);
            stmt.setShort(5, (short)qtyCache);
            stmt.setDate(6, dueDateCache);
	...
	if (debugging)
        ...
        try {
            conn = dataSource.getConnection();
            stmt = conn.prepareStatement(
                "UPDATE M_largeorder SET lo_o_id = ?, lo_ol_id = ?, " + 
                "lo_assembly_id = ?, lo_qty = ?, lo_due_date = ? WHERE lo_id = ? " +
                "AND lo_o_id = ? AND lo_ol_id = ? AND lo_assembly_id = ? " +
                "AND lo_qty = ? AND lo_due_date = ? ");

            int i = 1; 
            stmt.setInt(i++, salesOrderId);
            stmt.setInt(i++, orderLineNumber);
            stmt.setString(i++, assemblyId);
            stmt.setShort(i++, (short)qty);
            ...
            stmt.setInt(i++, salesOrderIdCache);
            stmt.setInt(i++, orderLineNumberCache);
            stmt.setString(i++, assemblyIdCache);
            stmt.setShort(i++, (short)qtyCache);
            stmt.setDate(i++, dueDate);
            ...
            if(ret != 1) {
                if(debugging) {
                     debug.println(1,"Optimistic concurrency control failed " + 
                                     "in LargeOrderEnt.ejbStore() for id = " + id);
                }
