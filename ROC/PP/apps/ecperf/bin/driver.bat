...
set ECPERF_HOME=C:\jboss\ecperf-3.0\ecperf
REM
...
set J2EE_HOME=C:\jboss\jboss-3.0\jboss-all\build\output\jboss-3.0.4RC1
...
set JNDI_CLASS=org.jnp.interfaces.NamingContextFactory
...
REM
REM For example, to specify localhost use the following URL iiop://localhost:900    
set NAMING_PKGS=org.jboss.naming:org.jnp.interfaces
If %NAMING_PROVIDER%=="None Specified" (
  Echo You must specify a JNDI naming provider.class for the driver to work.
  Echo For example, to specify localhost use the following URL iiop://localhost:900
  Echo Current setting is NAMING_PROVIDER=%NAMING_PROVIDER%
...
REM                  |
REM                  \/
REM set CLASSPATH=%ECPERF_HOME%\jars\ECPerf.jar;%J2EE_HOME%\lib\j2ee.jar;%ECPERF_HOME%\jars\ecperf-client.jar;%ECPERF_HOME%\jars\driver.jar

for %%i in (%ECPERF_HOME%\jars\*.jar) do call lcp.bat %%i
for %%i in (%J2EE_HOME%\client\*.jar) do call lcp.bat %%i
...
start %JAVA_HOME%\bin\java -Xmx100m -Djava.naming.factory.initial=%JNDI_CLASS% -Djava.naming.provider.url=%NAMING_PROVIDER% -Djava.naming.factory.url.pkgs=%NAMING_PKGS% -Djava.security.policy=%DRIVER_POLICY%  -Dorg.omg.CORBA.ORBInitialHost=%SUT_MACHINE%  %DRIVER_PACKAGE%.MfgAgent %CONFIG_DIR%/agent.properties M1 %DRIVER_MACHINE% > out.txt 2>&1

start %JAVA_HOME%\bin\java -Xmx100m -Djava.naming.factory.initial=%JNDI_CLASS% -Djava.naming.provider.url=%NAMING_PROVIDER% -Djava.naming.factory.url.pkgs=%NAMING_PKGS% -Djava.security.policy=%DRIVER_POLICY%  -Dorg.omg.CORBA.ORBInitialHost=%SUT_MACHINE%  %DRIVER_PACKAGE%.LargeOLAgent %CONFIG_DIR%/agent.properties L1 %DRIVER_MACHINE%

start %JAVA_HOME%\bin\java -Xmx100m -Djava.naming.factory.initial=%JNDI_CLASS% -Djava.naming.provider.url=%NAMING_PROVIDER% -Djava.naming.factory.url.pkgs=%NAMING_PKGS% -Djava.security.policy=%DRIVER_POLICY% -Dorg.omg.CORBA.ORBInitialHost=%SUT_MACHINE%  %DRIVER_PACKAGE%.OrdersAgent %CONFIG_DIR%/agent.properties O1 %DRIVER_MACHINE%
Pause

%JAVA_HOME%\bin\java -Djava.naming.factory.initial=%JNDI_CLASS% -Djava.naming.provider.url=%NAMING_PROVIDER% -Djava.naming.factory.url.pkgs=%NAMING_PKGS% %DRIVER_PACKAGE%.Driver %CONFIG_DIR%/run.properties
