...
public class RequestGenerator
{
     ...
     */
    public RequestGenerator( String filename )
	...
	RubisUserRequest nextReq;
	
	if( nextState == TransitionFunction.HOME )
	{ 
	    nextReq = genHome( nextState );
	    return nextReq;
	}
	else if( nextState == TransitionFunction.REGISTER )    nextReq = genRegister( nextState );
	else if( nextState == TransitionFunction.REG_USER )    nextReq = genRegisterUser( nextState );
	else if( nextState == TransitionFunction.BROWSE )      nextReq = genBrowse( nextState );
	else if( nextState == TransitionFunction.BROWSE_CAT )  nextReq = genBrowseCategories( nextState );
	else if( nextState == TransitionFunction.SEARCH_CAT )  nextReq = genSearchItemsInCategory( nextState, response );
	else if( nextState == TransitionFunction.BROWSE_REG )  nextReq = genBrowseRegions( nextState );
	else if( nextState == TransitionFunction.BR_CAT_REG )  nextReq = genBrowseCategoriesInRegion( nextState );
	else if( nextState == TransitionFunction.SEARCH_REG )  nextReq = genSearchItemsInRegion( nextState, response );
	else if( nextState == TransitionFunction.VIEW_ITEM )   nextReq = genViewItem( nextState, response );
	else if( nextState == TransitionFunction.VIEW_USER )   nextReq = genViewUserInfo( nextState, response );
	else if( nextState == TransitionFunction.VIEW_HIST )   nextReq = genViewBidHistory( nextState, response );
	else if( nextState == TransitionFunction.BUY_NOW )     nextReq = genBuyNow( nextState, response );
	else if( nextState == TransitionFunction.STORE_BUY )   nextReq = genStoreBuyNow( nextState, response );
	else if( nextState == TransitionFunction.PUT_BID )     nextReq = genPutBid( nextState, response );
	else if( nextState == TransitionFunction.STORE_BID )   nextReq = genStoreBid( nextState, response );
	else if( nextState == TransitionFunction.PUT_COMMENT ) nextReq = genPutComment( nextState, response );
	else if( nextState == TransitionFunction.STORE_COMM )  nextReq = genStoreComment( nextState, response );
	else if( nextState == TransitionFunction.SELECT_CAT )  nextReq = genSelectCategorySellItem( nextState );
	else if( nextState == TransitionFunction.SELL_ITEM )   nextReq = genSellItemForm( nextState );
	else if( nextState == TransitionFunction.REG_ITEM )    nextReq = genRegisterItem( nextState );
	else if( nextState == TransitionFunction.ABOUT_ME )    nextReq = genAboutMe( nextState );
	else if( nextState == TransitionFunction.LOGIN )       nextReq = genLogin( nextState );
	else if( nextState == TransitionFunction.LOGIN_USR )   nextReq = genLoginUser( nextState, prevRequest.getEngineId() );
	else if( nextState == TransitionFunction.LOGOUT )      nextReq = genLogout( nextState );
	else if( nextState == TransitionFunction.BACK)	       nextReq = REQ_BACK;
	else if( nextState == TransitionFunction.ABANDON )     nextReq = REQ_ABORT;
	else throw new RuntimeException( "Should never get here" );

	if( nextReq!=REQ_BACK && nextReq!=REQ_ABORT )
	{
	    nextReq.setParentSession( prevRequest.getParentSession() );
	}
     ...
     */
    private String genUrl( String servletNameAndArgs )
    {
	try {
	    return new String( "ejb_rubis_web/" + servletNameAndArgs );
	}
	catch( Exception e )
	{
	    e.printStackTrace();
    ...
    {
	String url = genUrl( "register.html" );
	...
	String url = genUrl( "edu.rice.rubis.beans.servlets.RegisterUser" +
			  "?firstname=" + firstName + 
			  "&lastname="  + lastName  +
			  "&nickname="  + nickName  +
			  "&email="     + email     +
    ...
    {
	String url = genUrl( "edu.rice.rubis.beans.servlets.BrowseCategories" );
	return new RubisUserRequest( url, MID_ACTION, state );
    }
	...
	String url = genUrl( "edu.rice.rubis.beans.servlets.SearchItemsByCategory" +
    ...
    {
	String url = genUrl( "edu.rice.rubis.beans.servlets.BrowseRegions" );
	return new RubisUserRequest( url, MID_ACTION, state );
    }
	...
	String url = genUrl( "edu.rice.rubis.beans.servlets.SearchItemsByRegion" +
    ...
    private RubisUserRequest genViewItem( RubisUserState state, HttpResponse response )
    {
	 ...
	 String url = genUrl( "edu.rice.rubis.beans.servlets.ViewItem" +
			   "?itemId=" + itemId );
	...
	String url = genUrl( "edu.rice.rubis.beans.servlets.ViewUserInfo" +
			  "?userId=" + userId );
    ...
    {
	String url = genUrl( "edu.rice.rubis.beans.servlets.ViewBidHistory" +
    ...
    {
	String url = genUrl( "edu.rice.rubis.beans.servlets.BuyNow" +
			  "?itemId=" + extractItemId( response ) );
	...
	String url = genUrl( "edu.rice.rubis.beans.servlets.StoreBuyNow" +
			  "?itemId=" + extractItemId( response ) +
	...
	String url = genUrl( "edu.rice.rubis.beans.servlets.PutBid" +
			  "?itemId=" + itemId );
	...
	String url = genUrl( "edu.rice.rubis.beans.servlets.StoreBid" +
			  "?itemId=" + extractItemId( response ) +
    ...
    {
	String url = genUrl( "edu.rice.rubis.beans.servlets.PutComment" +
			  "?itemId=" + extractItemId( response ) +
	...
	String url = genUrl( "edu.rice.rubis.beans.servlets.StoreComment" +
			  "?itemId=" + extractItemId( response ) +
	...
	String url = genUrl( "edu.rice.rubis.beans.servlets.BrowseCategories" ); 
	*/
	String url = genUrl( "edu.rice.rubis.beans.servlets.BrowseCategories?nickname=&password=" ); 
	...
	String url = genUrl( "edu.rice.rubis.beans.servlets.SellItemForm" +
			  "?category=" + (1 + categoryId) );
    ...
    private RubisUserRequest genRegisterItem( RubisUserState state )
    {
	 ...
	 String url = genUrl( "edu.rice.rubis.beans.servlets.RegisterItem" +
			   "?name=" + itemName +
    ...
    {
	String url = genUrl( "edu.rice.rubis.beans.servlets.AboutMe" );
    ...
    {
	String url = genUrl( "login.html" );
	return new RubisUserRequest( url, MID_ACTION, state );
    }

    /*-- LoginUser --*/
    private RubisUserRequest genLoginUser( RubisUserState state, int engineId )
    {
	// this.loggedIn = true;
	...
	// userId = 1 + (new Random()).nextInt( Configuration.numUsersInDB );
	//
	// Now we just use our engine ID to compute the user ID

	int userId = Configuration.minUserId + engineId;
	assert userId<=Configuration.maxUserId : "userId=" + userId + " maxUserId=" + Configuration.maxUserId;

	String url = genUrl( "edu.rice.rubis.beans.servlets.Login" +
    ...
    {
	String url = genUrl( "edu.rice.rubis.beans.servlets.Logout" );
	return new RubisUserRequest( url, END_ACTION, state );
    }
