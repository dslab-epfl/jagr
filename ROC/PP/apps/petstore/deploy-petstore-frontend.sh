...
if [ -z $1 ]; then
  ...
  echo "    ./deploy-petstore-frontend.sh [configname]"
  exit 1;
fi;
...
###
cp $PETSTORE_HOME/jboss/cloudscape-service.xml $JBOSS_SERVER/$NEWCONFIG/deploy
...
###
## only deploy WAR file for the front-end nodes
cp $PETSTORE_HOME/src/apps/petstore/build/petstore.war $JBOSS_SERVER/$NEWCONFIG/deploy
...
###
echo Done creating Petstore frontend configuration\: $NEWCONFIG
