...
<TR>
	...
	<TD colspan=2>
		<FORM METHOD=POST ACTION="new_order_ssbean.jsp">

		&nbsp;
		<center>
		<br><b><font size=+4>New Order </font> </b> </center>
		<center> <font size=+1>(Using Stateful Session Bean)</font></center>

		<br><font size=+2></font>&nbsp;
		<CENTER>
		<font size=+3>Please enter customer number and select items in the list
		</font> <br> <br> 

		<%@ page language="java" import="com.sun.ecperf.webbeans.*" errorPage="error.jsp" %>
		<jsp:useBean id='cartBean' scope='session' class='com.sun.ecperf.webbeans.CartBean' />
		...
		<%
				if (submit_value.equals("Add")) {
					cartBean.addItem(request.getParameter("item_name"), request.getParameter("qty"));
				} else if (submit_value.equals("Remove")) {
					cartBean.removeItem(request.getParameter("item_name"), request.getParameter("qty"));
				} else if (submit_value.equals("Order")) {
					cartBean.setCustomerID(request.getParameter("cust_id"));
		%>
					<jsp:forward page="submit_order_ssbean.jsp" />
		<%
				}
				...
				String item_id;
				int item_qty;
				customer_list = cartBean.getCustomerList();
		...
		<%
				...
				cartBean.resetCustomerList();
			}

			java.util.Vector items_list;
			String item;

			items_list = cartBean.getItemsList();
			if (items_list.size() > 0) {
		%>
		<font size=+2>Customer Number</font> <INPUT NAME="cust_id" TYPE=text SIZE="4" VALUE=<%= request.getParameter("cust_id")!= null ? request.getParameter("cust_id") : ""  %> > 
		<BR><BR>


		<font size=+2>Please choose items to order <br><br> </font>
		...
		<br><br> <font size=+2>	<p><INPUT TYPE=submit name="submit" value="Order"> </font>
