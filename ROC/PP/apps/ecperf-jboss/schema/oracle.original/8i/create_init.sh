...
#
# $Id: create_init.sh,v 1.1.1.1 2002/09/18 03:13:11 patriot1burke Exp $
#
# create_init.sh <db>
...
if [ $# -ne 1 ]
then
	echo "Usage: create_init.sh <database_name>"
	exit 1
fi
...
chmod u+w $T
sed "s/DBNAME/$DB/" $T | sed "s'cntrl.dbf'${ORACLE_HOME}/dbs/cntrl${DB}.dbf'" \
