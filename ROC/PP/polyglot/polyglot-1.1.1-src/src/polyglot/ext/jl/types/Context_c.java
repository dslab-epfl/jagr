...
public class Context_c implements Context
{
    ...
    public Collection getPublicProtectedVariables() {
	Collection ret;
	
	if( outer != null ) {
	    ret = outer.getPublicProtectedVariables();
	}
	else {
	    // avoid null-checks later by always returning something...
	    ret = new HashSet();
	}
	...
	if( vars != null ) {
		...
		if( !vi.flags().isPrivate() && !vi.flags().isPackage()) {
		    ret.add( vi );
		}
	    }
	}
