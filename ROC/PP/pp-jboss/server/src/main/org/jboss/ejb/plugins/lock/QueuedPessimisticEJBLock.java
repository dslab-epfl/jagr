...
{
   protected HashMap txLocks = new HashMap();
   protected LinkedList txWaitQueue = new LinkedList();
   protected boolean isReadOnlyTxLock = true;

   protected int txIdGen = 0;
   ...
   protected boolean waitForTx(Transaction miTx, boolean trace) throws Exception
   {
             ...
             !getTransaction().equals(miTx))
      {
         ...
         // Only queue the bean lock on the first iteration
         if (txLock == null)
         ...
         synchronized(txLock)
         {
            ...
            {
               log.warn("wait() interrupted in txLock, possibly a timeout");
            }
