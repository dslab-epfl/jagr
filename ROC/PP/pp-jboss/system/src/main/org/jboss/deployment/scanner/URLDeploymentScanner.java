   ...
   implements DeploymentScanner, URLDeploymentScannerMBean
{
   ...
   public void setURLComparator(String comparatorClassName)
   {
      try
      {
         sorter = (Comparator)Thread.currentThread().getContextClassLoader().loadClass(comparatorClassName).newInstance();
      }
      catch (Exception e)
      {
   ...
   public void setFilter(String filterClassName)
   {
      try
      {
         filter = (FileFilter)Thread.currentThread().getContextClassLoader().loadClass(filterClassName).newInstance();
      }
      catch (Exception e)
      {
         log.warn("Unable to create URLComparator.", e);
