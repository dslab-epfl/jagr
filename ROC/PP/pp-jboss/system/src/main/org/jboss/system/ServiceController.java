   ...
   implements ServiceControllerMBean, MBeanRegistration, NotificationListener
{
    ...
    *
    */
   private final Map classNameToMBeansMap = Collections.synchronizedMap(new HashMap());
    ...
    *
    */
   private Map waitingConfigs = Collections.synchronizedMap(new HashMap());
   ...
   */
   ...
   {
      List waitingConfigsCopy = null;
      synchronized( waitingConfigs )
      {
         waitingConfigsCopy = new ArrayList(waitingConfigs.keySet());
      }
      return waitingConfigsCopy;
   }
   ...
   * @jmx:managed-operation
   */
   public List install(Element config, ObjectName loaderName)
      throws DeploymentException
   {
      List mbeans = null;
      synchronized( this )
      {
         mbeans = configurator.install(config, loaderName);
      }
   ...
   * @jmx:managed-operation
   */
   public void create(ObjectName serviceName, Collection depends)
      throws Exception
   {
      ...
      try
      {
         ...
         if (iterator.hasNext()) {
            log.debug("Starting dependent components: " + ctx.dependsOnMe);
         }

         while (iterator.hasNext())
         {
            // marcf fixme circular dependencies?
            create(((ServiceContext) iterator.next()).objectName);
         }
      }
      finally {
         NDC.pop();
   ...
   */
   public void start(ObjectName serviceName) throws Exception
   {
      ...
      try
      {
         ...
         {
            ServiceContext ctx2 = (ServiceContext) iterator2.next();
   ...
   */
   public void remove(ObjectName objectName) throws Exception
   {
      ...
      for (Iterator iterator = ctx.iDependOn.iterator(); iterator.hasNext();)
      {
             ...
             && iDependOnContext.dependsOnMe.size() == 0)
         {
            ...
            if (debug)
               log.debug("Removing context for non existent service it is " +
   ...
   {
      List servicesCopy = null;
      synchronized( installedServices )
      {
         servicesCopy = new ArrayList(installedServices);
      }
      log.info("Stopping " + servicesCopy.size() + " services");
   ...
   */
   ...
   public void registerMBeanClassName(ObjectInstance instance)
   {
      String className  = instance.getClassName();
      Set mbeans = null;
      synchronized( classNameToMBeansMap )
      {
         mbeans = (Set)classNameToMBeansMap.get(className);
         if (mbeans == null)
         {
            mbeans = new HashSet();
   ...
   public void handleNotification(Notification notification, Object handback)
   {
      ...
      {
         ClassLoader ucl = (ClassLoader) notification.getUserData();
   ...
   ServiceContext createServiceContext(ObjectName objectName)
   {
      ServiceContext ctx = null;
      synchronized( nameToServiceMap )
      {
         // If it is already there just return it
         if ( nameToServiceMap.containsKey(objectName) == true )
         {
            ctx = (ServiceContext)nameToServiceMap.get(objectName);
         }
         else
         {
   ...
   private void newClassLoaderNotification()
   {
      ...
      synchronized ( waitingConfigs )
      {
         waitingConfigs = new HashMap(waitingConfigs);
         waitingConfigs.clear();
      }
