       ...
       extends JBossTestCase
{
   ...
   public void testAddRemoveConnectionManager() throws Exception
   {
      ...
      //check it isn't there already
      assertTrue("test mbean already registered", !getServer().isRegistered(mcfName));
      //try to create the RARDeployment.
      assertTrue("server did not return an objectInstance",
            getServer().createMBean("org.jboss.resource.connectionmanager.RARDeployment",
            mcfName) != null);

      //check deployment registered RARDeployment
      assertTrue("test mbean not registered", getServer().isRegistered(mcfName));
      ...
      //check it isn't there already
      assertTrue("test mbean already registered", !getServer().isRegistered(mcpName));
      //try to create the pool.
      assertTrue("server did not return an objectInstance",
            getServer().createMBean("org.jboss.resource.connectionmanager.JBossManagedConnectionPool",
            mcpName) != null);

      //check deployment registered connection factory loader
      assertTrue("test mbean not registered", getServer().isRegistered(mcpName));
      ...
      /*
      assertTrue("State is not started", "Started".equals(getServer().getAttribute(cmName, "StateString")));
      //invoke(cmName, "start", args, sig);
