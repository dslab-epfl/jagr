...
public class PolicyUnitTestCase extends JBossTestCase
{
   ...
   /** Test that the current thread cannot be modified. This test does not
    work because the only time the RuntimePermission("modifyThread") is
    checked in when the calling thread is a member of the root ThreadGroup,
    and this is only the case for threads started by the VM it appears.
    */
