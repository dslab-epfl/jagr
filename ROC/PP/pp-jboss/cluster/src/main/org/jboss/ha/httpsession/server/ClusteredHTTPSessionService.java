   ...
   implements ClusteredHTTPSessionServiceMBean
{
   ...
   public SerializableHttpSession getHttpSession (String sessionId) throws EJBException
   {
      if (log.isDebugEnabled ())
   ...
   protected ClusteredHTTPSessionBusiness createSession (String id) throws Exception
   {
      try
      {
         ...
         if (useLocalBean)
         {
            sessionBean = localHttpSessionHome.create (id);
         }
         else
         {
            sessionBean = httpSessionHome.create (id);
         }
         return sessionBean;
      }
      catch (Exception e)
