   ...
   implements Invoker, Externalizable
{
      ...
      throws Exception
   {
      ...
      for (int i = 0; i < MAX_RETRIES; i++)
      {
		 ...
		 mi.setValue( "roc.requestinfo",
			      roc.pinpoint.tracing.ThreadedRequestTracer.getRequestInfo(),
			      org.jboss.invocation.PayloadKey.AS_IS );
	     }
		 ...
		 roc.pinpoint.tracing.TracingObjectWrapper resp = 
		     (roc.pinpoint.tracing.TracingObjectWrapper)result.get();
		 
		 roc.pinpoint.tracing.ThreadedRequestTracer.getRequestInfo().setSeqNum( resp.getRequestSeqNum() );
		 return resp.get();
		 // TODO: need to grab seq num even if an exception is thrown!!
	     }
	     // ROC PINPOINT EMK END
	     else {
		 return result.get();
