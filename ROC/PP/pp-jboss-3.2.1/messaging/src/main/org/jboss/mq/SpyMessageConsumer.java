   ...
   implements MessageConsumer, SpyConsumer, Runnable
{
      ...
      if (session.transacted)
      {
         ...
         if (anonymousTXID != null)
         {
            if (session.getCurrentTransactionId() == anonymousTXID)
            {
               ...
               {
                  session.unsetCurrentTransactionId();
               }
      ...
      throws JMSException
   {
      ...
      // ROC PINPOINT EMK begin   28 LINES
      if( roc.config.ROCConfig.ENABLE_PINPOINT &&
	  roc.config.ROCConfig.ENABLE_PINPOINT_TRACING_JMS ) {
	  String reqIdSender = message.getStringProperty( "roc.pinpoint.tracing.requestid" );
	  int reqSeqNumSender = message.getIntProperty( "roc.pinpoint.tracing.requestseqnum" );
	  if(reqIdSender == null) {
	      System.err.println( "roc.pinpoing.tracing JMS: received " + 
				  "message without requestinfo object" );
	  }
	  else {
	      // create a new requestid locally.
	      roc.pinpoint.tracing.ThreadedRequestTracer.setRequestInfo( null );
	      roc.pinpoint.tracing.RequestInfo reqInfoRecvr =
		  roc.pinpoint.tracing.ThreadedRequestTracer.getRequestInfo();

	      // TODO: report this as an atomic call from a message queue?

	      // report a link between the sender and receiver paths
	      java.util.Map originInfo = new java.util.HashMap();
	      originInfo.put( "linktoreqid", reqInfoRecvr.getRequestId() );
	      originInfo.put( "type", roc.pinpoint.tracing.TracingHelper.TYPE_JMS );
	      roc.pinpoint.tracing.Observation obs = 
		  new roc.pinpoint.tracing.Observation( roc.pinpoint.tracing.Observation.EVENT_LINK,
							reqIdSender,
							reqSeqNumSender,
							originInfo,
							null, null );
	      roc.pinpoint.tracing.GlobalObservationPublisher.Send( obs );
	  }
      }
