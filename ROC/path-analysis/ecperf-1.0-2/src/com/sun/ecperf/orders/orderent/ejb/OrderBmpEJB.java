 ...
 *
 * $Id: OrderBmpEJB.java,v 1.1.1.1 2002/11/16 05:35:28 emrek Exp $
 *
 *
 */
...
public class OrderBmpEJB extends OrderCmpEJB {
    ...
    public Integer ejbCreate(int customerId, ItemQuantity[] quantities)
        ...
        try {
            ...
            if (ret != 1) {
                if(debugging)
                    debug.println(1, "executeUpdate() returned " + ret
                                  + " for :");
            if(debugging)
                debug.println(1, "  INSERT INTO O_orders(" + id + ", "
                              + customerId + ", " + orderLineCount + ", "
                              + discount + ", " + total + ", " + orderStatus
                              + ", " + entryDate + ", " + shipDate + ")");

                throw new CreateException("INSERT INTO O_orders(" + id + ", "
                                          + customerId + ", "
            ...
            if(debugging)
                debug.println(1, "SQLException in INSERT INTO O_orders(" + id
                          + ", " + customerId + ", " + orderLineCount + ", "
                          + discount + ", " + total + ", " + orderStatus
                          + ", " + entryDate + ", " + shipDate + ")");
    ...
    public Integer ejbFindByPrimaryKey(Integer key) throws FinderException {
        ...
        try {
            ...
            statement  = connection.prepareStatement(
                "SELECT o_id FROM O_orders  WHERE o_id = ?");
            ...
            if (!resultSet.next()) {
                if(debugging)
                    debug.println(2, "O_orders row not found for o_id : " + key);
            ...
            if(debugging)
                debug.println(1, "SQLException in find by primary key for o_id : " + key);
    ...
    public java.util.Enumeration ejbFindByCustomer(int cid)
        ...
        try {
            connection = dataSource.getConnection();
            statement  = connection.prepareStatement(
                "SELECT o_id FROM O_orders  WHERE o_c_id = ?");
            ...
            if(debugging)
                debug.println(1, "SQLException in SELECT orders by customer "
                              + cid);
            debug.printStackTrace(e);
    ...
    public void ejbRemove() throws RemoveException {
        ...
        try {
            ...
            statement  =
                connection
                    .prepareStatement("DELETE FROM O_orders WHERE o_id = ?");
	      ...
	      if(debugging) {
                    debug.println(1, "executeUpdate() returned " + ret
                                   + " for :");
                    debug.println(1, "DELETE from O_orders WHERE o_id = " + id);
	      }
            ...
            if(debugging)
                debug.println(
                1, "SQLException in DELETE from O_orders WHERE o_id = " + id);
            debug.printStackTrace(e);
    ...
    public void ejbLoad() {
        ...
        try {
            ...
            if (!resultSet.next()) {
                if(debugging)
                    debug.println(1, "O_orders row not found for o_id = " 
                                     + pkey);
            ...
            if(debugging)
                debug.println(
                    1, "SQLException in SELECT FROM O_orders WHERE o_id = "
                    + pkey);
            debug.printStackTrace(e);
    ...
    public void ejbStore() {
        ...
        try {
            ...
            if (ret != 1) {
                if(debugging)
                    debug.println(1, "executeUpdate() returned " + ret
                                  + " in ejbStore() of O_orders");

                throw new NoSuchEntityException("O_orders " + id);
            }
        } catch (SQLException e) {
            if(debugging)
                debug.println(1, "SQLException in UPDATE O_orders row " + id
                              + " SET o_c_id = " + customerId + ", o_ol_cnt = "
                              + orderLineCount + ", o_discount = " + discount
                              + ", o_total = " + total + ", o_status = "
                              + orderStatus + ", o_entry_date = " + entryDate
                              + ", o_ship_date = " + shipDate);
            debug.printStackTrace(e);
