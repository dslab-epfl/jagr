...
// $Id: ServletHolder.java,v 1.8 2003/02/09 00:22:30 emrek Exp $
// ---------------------------------------------------------------------------
 ...
 * requested.
 *
 * @version $Id: ServletHolder.java,v 1.8 2003/02/09 00:22:30 emrek Exp $
 * @author Greg Wilkins
 */
    ...
    implements Comparable
{
	...
	//// pull out the request ID and seq no from request
	Boolean seenThisRequest = (Boolean)(request.getAttribute("ROC_TRACER_SEEN_THIS"));
	roc.pinpoint.tracing.RequestInfo PP_reqInfo = null;
	if (seenThisRequest == null) {
	    String reqId = ((HttpServletRequest)request).getHeader("TRACER_REQUEST_ID");
	    int seqNum = Integer.parseInt(((HttpServletRequest)request).getHeader("TRACER_REQUEST_SEQ"));
	    
	    PP_reqInfo = new roc.pinpoint.tracing.RequestInfo( reqId );
	    PP_reqInfo.setSeqNum( seqNum );
	    roc.pinpoint.tracing.ThreadedRequestTracer.setRequestInfo( PP_reqInfo );

	    request.setAttribute("ROC_TRACER_SEEN_THIS", Boolean.TRUE);
	}

	Map PP_originInfo = new HashMap( roc.pinpoint.tracing.java.EnvironmentDetails.GetDetails() );
	PP_originInfo.put( "name", _class.toString() );
	PP_originInfo.put( "type", "servlet" );

	Map PP_attributes = new HashMap();
	...
	//// ROC PINPOINT MIKECHEN EMK END   ////
	    ...
	    // [original code] servlet.service(request,response);
	    // [original code] servlet_error=false;
        }
        catch(UnavailableException e)
        {
