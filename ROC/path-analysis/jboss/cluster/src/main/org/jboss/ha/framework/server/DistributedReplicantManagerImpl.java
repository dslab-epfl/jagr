 ...
 * </ol>
 * <p><b>2002/08/27: Sacha Labourey</b>
 * <ol>
              ...
              HAPartition.HAPartitionStateTransfer
{
   ...
   public void add(String key, Serializable replicant) throws Exception
   {
      ...
      Object[] args =
      {key, this.nodeName, replicant};
      partition.callAsynchMethodOnCluster(SERVICE_NAME, "_add", args, true);
   ...
   protected void mergeMembers()
   {
      try
      {
         log.debug("Start MergeMembers in DRM");
         ...
         if (rsp.size() == 0) 
            log.info("No responses from lookupLocalReplicants!");
         else 
         { 
            log.info("lookupLocalReplicants size() = " + rsp.size()); 
         }
