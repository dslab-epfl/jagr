...
// $Id: HttpConnection.java,v 1.1 2004/06/08 13:33:05 emrek Exp $
// ========================================================================
 ...
 * @see HttpListener
 * @see HttpServer
 * @version $Id: HttpConnection.java,v 1.1 2004/06/08 13:33:05 emrek Exp $
    ...
    implements OutputObserver
{
	...
	if( ROCConfig.ENABLE_PINPOINT &&
	    ...
	    roc.pinpoint.tracing.TracingHelper.ReportException(
	        "org.mortbay.http.HttpConnection",
		roc.pinpoint.tracing.TracingHelper.TYPE_URL,
		e );
	}
	...
	if( ROCConfig.ENABLE_PINPOINT &&
	    ...
	    // make sure that the request info is reset for the new request
	...
	//// ROC PINPOINT MIKECHEN EMK END   ////
		...
		try {
		    ...
		    PP_reportURL = true;
		    roc.pinpoint.tracing.TracingHelper.ReportURL(
			"org.mortbay.http.HttpConnection",
			PP_name,
			requestclassifier,
			true /* BEGIN */ );

		    roc.pinpoint.tracing.RequestInfo ri =
			roc.pinpoint.tracing.ThreadedRequestTracer.getRequestInfo();
		    _response.setField("PP-Request", ri.getRequestId());
		}
		catch (Exception e) {
		    e.printStackTrace();
		}
