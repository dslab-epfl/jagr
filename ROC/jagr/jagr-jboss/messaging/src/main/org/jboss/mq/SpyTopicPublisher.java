...
public class SpyTopicPublisher
   ...
   protected void internalPublish(Topic topic, Message message, int deliveryMode, int priority, long timeToLive )
      ...
      // ROC PINPOINT EMK begin          8 LINES
      if( roc.config.ROCConfig.ENABLE_PINPOINT &&
	  roc.config.ROCConfig.ENABLE_PINPOINT_TRACING_JMS ) {

	  roc.pinpoint.tracing.TracingHelper.ReportAtomicMethod(
	      "org.jboss.mq.SpyTopicPublisher",
	      roc.pinpoint.tracing.TracingHelper.TYPE_JMS,
	      topic.getTopicName() );

	  message.setStringProperty( "roc.pinpoint.tracing.requestid",
				     roc.pinpoint.tracing.ThreadedRequestTracer.getRequestInfo().getRequestId() );
	  message.setIntProperty( "roc.pinpoint.tracing.requestseqnum",
				  roc.pinpoint.tracing.ThreadedRequestTracer.getRequestInfo().getSeqNum() );
      }
