...
# $Id: build.sh,v 1.1 2004/06/08 13:33:04 emrek Exp $
...
###
## ROC CONFIG EMK
## Before building JBoss, make sure that class roc.config.ROCConfig in
## build/roc-config/ has been compiled.
## If build.sh is given any options (e.g., clean), then don't do this.
###

if [ -z $JBOSS_TOP ]; then
    echo JBOSS_TOP not defined
    exit;
fi
...
if [ "x$1" == "x" ]; then
    echo Compiling ROC config file...
    $JAVA_HOME/bin/javac $JBOSS_TOP/build/roc-config/roc/config/ROCConfig.java
elif [ "$1" == "clean" ]; then
    echo Cleaning ROC config file...
    rm $JBOSS_TOP/build/roc-config/roc/config/ROCConfig.class
fi
