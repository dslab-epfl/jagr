   ...
   implements InvokerHA
{
      ...
      throws Exception
   {
	 ...
	 if( roc.config.ROCConfig.ENABLE_PINPOINT &&
		 ...
		 invocation.getValue( "roc.requestinfo" );
	 ...
	 // ROC PINPOINT EMK END
	  ...
	  if( roc.config.ROCConfig.ENABLE_PINPOINT &&
		  ...
		  // PinpointWrappedJRMPException will automatically embed the tracing sequencenumber
		  throw new roc.jboss.pinpoint.PinpointWrappedJRMPException( exception );
	      }
	      catch( Error error ) {
		  // PinpointWrappedJRMPException will automatically embed the tracing sequencenumber
		  throw new roc.jboss.pinpoint.PinpointWrappedJRMPException( error );
	      }
	  }
	  else {
	      // END ROC PINPOINT EMK BEGIN
	      // original code
	      org.jboss.mx.util.JMXExceptionDecoder.rethrow(e);
	  }
