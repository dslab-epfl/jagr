...
public class CheckURLReachability extends RequestInterceptor {
    ...
    private static Logger log = Logger.getLogger( "CheckURLReachability" );
     ...
     */
    public Response invoke(Request req) throws AbortRequestException {
	...
	if( !httpreq.getMetadata().containsKey("start-session") ) {
		...
		if( !isReachable ) {
		    log.info( "[STATS] CheckURLReachAble FAILED: lastpage=" + 
				     laststringid + "[" + lastpinpointid + 
				     "] does not link to " + 
				     httpreq.url );
		    throw new AbortRequestException("URL: "+httpreq.url +" is not reachable");
		}
	    }
	}
    ...
    private void rememberResponsePage(HttpRequest req, HttpResponse resp) throws IOException, MalformedURLException {
	    ...
	    log.info("remembering urls from response page " + resp.toString() );
