...
public class SB_AboutMeBean implements SessionBean 
{
   ...
   * @since 1.1
   */
  public Object[] getAboutMe(String username, String password) throws RemoteException
  {
    int          uid = -1;
    Integer      userId;
    ...
    if (uid == -1) {
	r[1] = "You don't have an account on RUBiS!<br>You have to register first.<br>";
	return r;
    }
    r[0] = new Integer(uid);
  ...
  public String getAboutMe(Integer userId) throws RemoteException
  {
    StringBuffer html = new StringBuffer();
    ...
    try 
    {
      ...
      html.append(listBoughtItems(userId, iHome));
      html.append(listWonItems(userId, user));
      // html.append(listBids(userId, nusername, password, user, iHome));
      html.append(listBids(userId, null, null, user, iHome));
      html.append(listComments(cHome, userId, uHome));
  ...
  public String listItem(Integer userId, ItemLocalHome iHome) throws RemoteException
  {
    ...
    else
    {
      ...
      try
      {
        ...
        while (it.hasNext()) 
        {
	    /* If META-INF/SB_AboutMe/ejb-jar.xml does not have the
	     * transaction optimization, this will generate a separate
	     * SQL call for each item! */
  ...
  public String listComments(CommentLocalHome home, Integer userId, UserLocalHome uHome) throws RemoteException
  {
    ...
    try 
    {
      ...
      while (it.hasNext()) 
      {
        ...
        try
        {
	    if ( edu.rice.rubis.beans.Config.noNicknameLookup )
	    {
		userName = comment.getFromUserId() + " (optimized)";
	    }
	    else
	    {
  ...
  public String listBids(Integer userId, String username, String password, UserLocal user, ItemLocalHome iHome) throws RemoteException 
  {
    ...
    html = new StringBuffer(printUserBidsHeader());
      while (it.hasNext()) 
      {
        try
        {
	  /* FIXME: This causes each bid to be looked up with a
	   * seperate SQL statement!  Could be optimized by doing a
	   * join in an ejbSelect... */
  ...
  public String printItemUserHasBidOn(BidLocal bid, ItemLocal item, String username, String password) throws RemoteException
  {
	...
	return item.printItemUserHasBidOn(bid.getMaxBid())+"\"><IMG SRC=\""+BeanConfig.context+"/bid_now.jpg\" height=22 width=90></a>\n";
    }
