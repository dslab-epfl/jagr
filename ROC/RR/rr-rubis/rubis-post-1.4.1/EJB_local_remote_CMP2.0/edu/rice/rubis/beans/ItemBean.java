/*
 * $Id: ItemBean.java,v 1.5 2004/05/25 22:44:00 fjk Exp $
 */
...
public abstract class ItemBean implements EntityBean 
{
 ...
 /** 
  ...
  /**
   * Get the seller's nickname by finding the Bean corresponding to
   * the user, unless configured to return the userId instead (which
   * avoids expensive lookups).
   *
   * @return nickname
   * @since 1.0
   */
  ...
  public String printItem()
  {
    return "<TR><TD><a href=\""+BeanConfig.context+"/servlet/edu.rice.rubis.beans.servlets.ViewItem?itemId="+getId()+"\">"+getName()+
      ...
      "<TD>"+getEndDate()+
	//
	// go to PutBid directly instead of PutBidAuth  Feb/27/04 S.Kawamoto
  ...
  public String printItemDescriptionToBuyNow(int userId)
  {
    String result = "<TABLE>\n"+"<TR><TD>Quantity<TD><b><BIG>"+getQuantity()+"</BIG></b>\n"+
      ...
      //
      // go PutComment instead of PutCommentAuth Feb/26/04 S.Kawamoto
      ...
      getSellerNickname()+"</a> (<a href=\""+BeanConfig.context+"/servlet/edu.rice.rubis.beans.servlets.PutComment?to="+getSellerId()+"&itemId="+getId()+"\">Leave a comment on this user</a>)\n"+
