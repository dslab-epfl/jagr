...
public abstract class URLGenerator implements Cloneable
{
  ...
  /**
   * Returns the name of the Login User script according to the implementation (PHP, EJB or Servlets).
   *
   * @return Login User script name
   */
  public abstract String LoginUserScript();

  /**
   * Returns the name of the Logout script according to the implementation (PHP, EJB or Servlets).
   *
   * @return Logout script name
   */
  public abstract String LogoutScript();
  ...
  public void setWebSiteName(String host)
  {
    ...
    * return host name
    *
    * @return hostName to throw request.
    */
   public InetAddress getHost() 
   {
       ...
       try
       {
	   host = InetAddress.getByName(webSiteName);
       }
  ...
  /** URL to the page corresponding to the file and host.
   *
   * @param filename file name
   * @param host host to connect
   * @return the URL corresponding to the file
   */
  public URL genericHTMLFile(String filename, String hostname)
  {
    try
    {
      URL url = new URL(protocol, hostname, webSitePort, filename);
      return url;
    }
    catch (java.net.MalformedURLException e)
    {
  ...
  // ================== Login/Logout =====================
  // ==                                                 ==
  // ==      Added by Stanford ROC team Feb 2004        ==
  // ==                                                 ==
  // ==  NOTE: These are currently only implemented for ==
  // ==  the "EJB_local_remote_CMP2.0" version of RUBiS ==
  // =====================================================

  /** URL to the login user page of the web site.
   *
  ...
  public URL login()
  {
    try
    {
      URL url = new URL(protocol, webSiteName, webSitePort, HTMLPath+"/login.html");
      return url;
    }
    catch (java.net.MalformedURLException e)
    {
      System.out.println("Error while generating login page URL: "+e.getMessage());
      return null;
    }
  }

  /** Login a user. You must provide all information requested by the web page.
   *
   * @param nickname user's nick name (login name)
   * @param password user's password
   *
   * @return URL to use to login a user
   */
  public URL loginUser(String nickname, String password)
  {
    try
    {
      nickname   = URLEncoder.encode(nickname);
      ...
      URL url = new URL(protocol, webSiteName, webSitePort, 
			scriptPath+"/"+LoginUserScript()+"?nickname="+nickname+"&password="+password);
      return url;
    }
    catch (java.net.MalformedURLException e)
    {
      System.out.println("Error while generating login user page URL: "+e.getMessage());
      return null;
    }
  }
  ...
  public URL logout()
  {
    ...
    {
      URL url = new URL(protocol, webSiteName, webSitePort, 
			scriptPath+"/"+LogoutScript());
      return url;
    }
    catch (java.net.MalformedURLException e)
    {
      System.out.println("Error while generating logout page URL: "+e.getMessage());
      return null;
    }
  }
  ...
  /**
   * public clone interface. Is it only a shallow copy?
   * 
   * @return clone object for this instance
   */
  public Object clone()
  {
      try
