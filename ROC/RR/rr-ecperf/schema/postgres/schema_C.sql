...
(
	...
	c_balance	FLOAT8,
	c_credit	char(2),
	c_credit_limit	FLOAT8,
	c_ytd_payment	FLOAT8
);

CREATE INDEX C_c_idx ON C_customer (c_id);
...
CREATE  INDEX C_supp_idx ON C_supplier (supp_id);
...
CREATE  INDEX C_site_idx ON C_site (site_id);
...
CREATE  INDEX C_p_idx ON C_parts (p_id);
...
CREATE  INDEX C_r_idx on C_rule (r_id);
...
CREATE  INDEX C_d_idx on C_discount (d_id);
