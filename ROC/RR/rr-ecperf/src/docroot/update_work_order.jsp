...
<TR>
	...
	<TD colspan=2>
		...
		<center>
		<br><b><font size=+4>Update Work Order </font></b></center>
		...
		<%
			...
			if (order_id != null) {
				if (workOrderSes.updateWorkOrder(order_id) ) {
		%>
					<font size=+3>  Updated order <%= order_id.intValue() %></font>
					<BR><BR><BR><BR>
					<font size=+2> 
		...
		%>
					<font size=+3>  Work order <%= order_id.intValue() %> is in the 
					final stage. It can't be updated. 
					<BR> <BR>
					Would you like to complete the work order ?
					</font>
					<FORM METHOD=POST ACTION="complete_work_order.jsp" >
					<INPUT TYPE=hidden NAME="order_id" VALUE=<%= order_id.toString() %> >
					<INPUT TYPE=submit VALUE="Yes">
					</FORM>	
					<BR><BR><BR><BR><BR>
