...
# $Id: start.sh,v 1.8 2003/04/12 04:37:14 steveyz Exp $
#
...
exec $JBOSS_HOME/bin/deploy-petstore.sh >& $RUNDIR/PetStore.log &
...
if [ "$1" == "vanilla" ] ; then
    ...
    exec xterm -e $JAVA org.jboss.RR.TheBrain -o $JBOSS_TOP/server/src/main/org/jboss/RR/f-map.xml -vanilla $JBOSS_HOME/server/default/deploy/petstore.ear/ >& $RUNDIR/TheBrain.log &
else
#use full app restarts
    echo ">>> Starting TheBrain... (use uRBs)"
    exec xterm -e $JAVA org.jboss.RR.TheBrain -o $JBOSS_TOP/server/src/main/org/jboss/RR/f-map.xml $JBOSS_HOME/server/default/deploy/petstore.ear/ >& $RUNDIR/TheBrain.log &
fi
