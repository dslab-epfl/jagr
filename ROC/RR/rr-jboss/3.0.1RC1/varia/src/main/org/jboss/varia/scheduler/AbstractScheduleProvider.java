...
//AS      log.info( "addScheduler(), start date: " + pStart + ", period: " + pPeriod + ", repetitions: " + pRepetitions );
      try {  // CANDEA
         ...
         throw new RuntimeException("<<< RR-specific (AbstractScheduleProvider.addSchedule) >>> " + e.toString());
      }
   }
      ...
      throws JMException
   {
      try { // CANDEA
         ...
         throw new RuntimeException("<<< RR-specific (AbstractScheduleProvider.removeSchedule) >>> " + e.toString());
      }
   }
...
//AS         log.info( "stopService(), call unregisterProvider()" );
      ...
      // CANDEA start
         ...
         throw new RuntimeException( "<<< RR-specific (AbstractScheduleProvider.stopService) >>> " + e.toString() );
      }
   }
}
