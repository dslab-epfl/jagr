   ...
   implements EjbModuleMBean
{
   ...
   private FailureMonitorMBean failureMonitor;
   private FaultInjectorMBean faultInjector;
   ...
   /** 
    * The <code>geappmd</code> method returns an appmetadata
    * @return a <code>ApplicationMetaData</code> value
    * @exception Exception if an error occurs
    * @jmx:managed-operation
    */
   public Object geappmd() throws Exception{
        return appMetaData;
   }
 
   public void seappmd(Object appmd) throws Exception{
        this.appMetaData = appmd;
   }   
   ...
   /**
      * @jmx:managed-operation
      */
     public Collection grabContainers()
    {
       return containers.values();
    }
   ...
   public void createService() throws Exception 
   {
      ...
      serviceController = (ServiceControllerMBean) MBeanProxy.create(ServiceControllerMBean.class,
			   ...
			   server);
      failureMonitor = (FailureMonitorMBean) MBeanProxy.create(FailureMonitorMBean.class,
                                                               FailureMonitorMBean.OBJECT_NAME,
                                                               server);

      faultInjector = (FaultInjectorMBean) MBeanProxy.create(FaultInjectorMBean.class,
                                                               FaultInjectorMBean.OBJECT_NAME,
                                                               server);
...
//         ApplicationMetaData appMetaData = (ApplicationMetaData) deploymentInfo.metaData;
         ApplicationMetaData appMetaData1 = (ApplicationMetaData) deploymentInfo.metaData;
	 appMetaData = (Object)appMetaData1;


//   log.info("SETTING UP APP META DATA: " + appMetaData1.getUrl());
...
**********************/  
         ...
         for (Iterator beans = appMetaData1.getEnterpriseBeans(); beans.hasNext(); )

         {
            ...
            try 
            {
               Container container = createContainer(bean, deploymentInfo);
               addContainer(container);
               failureMonitor.newContainer(container);
            } 
            catch (Exception e) 
            {
	...
	// Add Container to Failure Injector
	    faultInjector.newContainer(con);
   ...
   throws Exception
   {
      ...
      container.setInstancePool( createInstancePool( conf, cl ) );
      //Set the bean Lock Manager
      container.setLockManager(createBeanLockManager(container, ((EntityMetaData) bean).isReentrant(),conf.getLockClass(), cl));
   ...
   */
   ...
   throws NamingException, DeploymentException
   {
      ...
      {   // Either the application has a security domain or the container has security setup
         ...
         {
            throw new DeploymentException("Could not find the security-domain specified for this container, name="+confSecurityDomain, e);
         }
         catch(Exception e)
         {
            throw new DeploymentException("Invalid security-domain specified for this container, name="+confSecurityDomain, e);
         }
      }
