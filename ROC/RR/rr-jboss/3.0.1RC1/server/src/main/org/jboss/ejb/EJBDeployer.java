   ...
   implements EJBDeployerMBean
{
   ...
   /** A proxy for the FailureMonitor MBean */
   ...
   */
   ...
   /**
    * Get a reference to the ServiceController and FailureMonitor
    */
   public void startService() throws Exception
   {
	 ...
	 MBeanProxy.create(ServiceControllerMBean.class,
      ...
      failureMonitor = (FailureMonitorMBean)
         MBeanProxy.create(FailureMonitorMBean.class,
   ...
   */
      ...
      throws DeploymentException
   {
      ...
      try
      {
         ...
         ObjectName ejbModule = new ObjectName(EjbModule.BASE_EJB_MODULE_NAME + ",url=" + urlname);
...
//	 ObjectName faultInjector = FaultInjector.OBJECT_NAME;
//         server.invoke(faultInjector, "addEjbModule", new Object[] { ejbModule }, new 
//                       String[] { ObjectName.class.getName() });


/***********  ADDED 10/15 *******/

	ApplicationMetaData appMD = (ApplicationMetaData)server.invoke(ejbModule, 
		"geappmd", new Object[0], new String[0]); 

   	for (Iterator beans = appMD.getEnterpriseBeans(); beans.hasNext(); )
   
         {
            BeanMetaData bean = (BeanMetaData) beans.next();

            //
            // Get the JNDI name (for global name space)
            //
            String name = bean.getJndiName();

            //
            // Here we need to notify the FailureMonitor that a new bean has been deployed
            //
         }
      }
      catch (Exception e)
      ...
      throws DeploymentException
   {
      try 
      {
 	  ...
 	  ApplicationMetaData appMD = (ApplicationMetaData)server.invoke(di.deployedObject,
                "geappmd", new Object[0], new String[0]);
        ...
        for (Iterator beans = appMD.getEnterpriseBeans(); beans.hasNext(); )
      
         {
            BeanMetaData bean = (BeanMetaData) beans.next();
            //
            // notify FailureMonitor that the bean has been undeployed
            //
         }
