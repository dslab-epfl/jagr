...
public class DeploymentInfo
{
   ...
   */
      ...
      throws DeploymentException
   {
      ...
      /* Send this info to the database (candea@stanford.edu) */
      roc.rr.Component comp;
      String type = this.shortName.replaceAll(".*\\.", "");
      if ( parent != null ) {
	  comp = new roc.rr.Component( this.url.toString(), this.shortName, type, parent.url.toString() );
      } else {
	  comp = new roc.rr.Component( this.url.toString(), this.shortName, type, (String)null );
      }
      roc.rr.afpi.DBUtil.storeComponent( comp );
   }
   ...
   public void cleanup()
   {
      ...
      {
         log.warn("Not deleting localUrl, it is null or not a copy: " + localUrl);
      }
      else if (Files.deleteContaining(localUrl.getFile()))
      {
         log.info("Cleaned Deployment: " + localUrl);
      }
      else
      {
         log.warn("Could not delete directory " + localUrl + " restart will delete it");
      }
      localUrl = null;
   }
