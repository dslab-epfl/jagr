   ...
   implements Deployer, MainDeployerMBean
{
   ...
   /** A reference to a JARDeployer used for manifest entry referecnes that
    * do not map to a deployer.
    */
   private SubDeployer jarDeployer;
   ...
   public void addDeployer(final SubDeployer deployer)
   {
      ...
      synchronized(deployers)
      {
         ...
         if( deployer instanceof JARDeployer )
            jarDeployer = deployer;
      }
   ...
   /**
    * Microreboot a set of components (EJBs only, for now)
    *
    * @param     jndiName   Set of JNDI names of comps to uRB
    * @exception DeploymentException
    * @exception MalformedURLException
    * @exception MissingResourceException if uRB-ing non-deployed component
    * @jmx:managed-operation
    */
   private static final boolean enableMicrorebootInProgress = false;

   public void microreboot( Set compSet )
       throws DeploymentException, 
	      MalformedURLException, 
	      MissingResourceException
   {
       // Build the set of JAR files we need to microreboot
       Set jarSet = new HashSet();
       for( Iterator i = compSet.iterator() ; i.hasNext() ; )
       {
	   String compName = (String) i.next();
	   DeploymentInfo di = ComponentMap.getDeploymentInfo( compName );
	   if( di == null )
	   {
	       throw new MissingResourceException( compName + " is not deployed", "", "" );
	   }
	   jarSet.add( di );
       }

       // Microreboot each JAR serially
       for( Iterator i = jarSet.iterator(); i.hasNext() ; )
       {
	   Iterator j = compSet.iterator();
	   DeploymentInfo di = (DeploymentInfo) i.next();

	   log.info( "Microrebooting " + di.url );

	   //
	   // If the microreboot-in-progress notification is not
	   // required, skip the whole JNDI bind stuff.
	   //
	   if( ! enableMicrorebootInProgress )
	   {
	       stop( di );
	       doDestroy( di, true );
	       deploy( di );
	   ...
	   //
	   // rebind JNDI name of EJB to MicrorebootInProgress object
	   //

	   InitialContext initialContext=null;
	   MicrorebootInProgress uRBInProgress 
	       = new MicrorebootInProgress();
	   String cName = ((String)j.next())+"Home";
	   try {
	       initialContext = new InitialContext();
	       initialContext.rebind(cName,uRBInProgress);
	       System.out.println("Again bind MicrorebootInProgress to JNDI name: "+cName);
	   } catch (Exception e) {
	       System.out.println("JNDI binding for microreboot failed: "+e);
	   }
	   ...
	   //
	   // The above doDestory method currently unbinds JNDI name.
	   // Thus the following code binds it to MicrorebootInProgress again.
	   // This is temporal implementation. Ideally we should privent 
	   // the above doDestroy method unbinding JNDI name.
	   //
	   try {
	       initialContext.bind(cName,uRBInProgress);
	       System.out.println("Again bind MicrorebootInProgress to JNDI name: "+cName);
	       j.hasNext();
	   } catch (Exception e) {
	       System.out.println("JNDI binding for microreboot failed: "+e);
    ...
    * Microreboot a JAR
    *
    * @param     urlSpec   URL of JAR to microreboot
    * @exception DeploymentException
    * @exception MalformedURLException
    * @jmx:managed-operation
    */
   public void microrebootJar( String urlspec )
      throws DeploymentException, MalformedURLException
   {
      ...
      try
      {
	  log.info( "Microrebooting " + sdi.url );
	  stop( sdi );
	  doDestroy( sdi, true );
      }
      catch ( NullPointerException nullE )
      {
	  log.error( "Cannot microreboot " + urlspec + " (not currently deployed)" );
	  return;
      }
      catch ( Throwable t )
      {
   ...
   private void doDestroy(DeploymentInfo di, boolean microReboot)
   {
	 ...
	 if (!microReboot)
	     di.cleanup(); // Nuke my stuff, this includes the class loader
