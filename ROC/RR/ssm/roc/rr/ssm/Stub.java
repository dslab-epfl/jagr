	...
	*/
    ...
    public Object read(Vector v) throws SystemOverloadedException,
        ...
        if (exp < System.currentTimeMillis()) {
            System.out.println("[Stub] cookie expire time "+exp);
            System.out.println("[Stub] current time "+
                               System.currentTimeMillis());
            System.out.println("[Stub] THE STATE HAS EXPIRED");
            return null;
        }
    ...
    /*
     * readWOCheck: This method doesn't check data corruption.
     *   When you read a data which is an java object 
     *     You should use this method instead of original read. 
     *   Because checking of hash code always produces an error
     *     whenever read data is an java object. 
     *                 Mar/17/2004  S.Kawamoto  
     */
    public Object readWOCheck(Vector v) throws SystemOverloadedException,
        RequestTimedOutException, BadChecksumException, InsufficientBricksAvailableException {
        long key = ( (Long) v.elementAt(0) ).longValue();
        int[] repliedbricks = (int[]) v.elementAt(1);
        long exp = ( (Long) v.elementAt(2) ).longValue();
    ...
    public Hashtable publishRequest(Msg r, int[] repliedbricks)
    throws SystemOverloadedException,
                RequestTimedOutException,
                InsufficientBricksAvailableException,
        BrickInfoIsStaleException {
        ...
        // s.kawamo
        //System.out.println("publishRequest: "+r);
        ...
        for (int p = 0; p < targetbricks.length; p++) {
            ...
            try {
                if (bi != null) {
                    synchronized(bi) {
                        // s.kawamoto
                        //System.out.println("trying to publish request for BrikInfo");
                        bi.publishRequest(r, force);
                        // s.kawamoto
                        //System.out.println("finished to publish request for BrikInfo");
                        force = true;
                    }
                    /*if (this._id == 5) {
        ...
        */
        try {
            synchronized (h) {
                int size = h.size();
                if (isWrite && size < _WQ || !isWrite && size < 1) {
                    //System.out.println("wait for "+_timeout);
                    h.wait(_timeout);
                    //System.out.println("finished to wait");
                }
            }
        } catch (Exception e) {
            //e.printStackTrace();
                ...
                while (true) {
                    ...
                    if (bi != null) {}
                        ...
                        try {
                            ...
                            l.start();
                            System.out.println("Started a new SSM listener thread for host " + host);
