...
then
    ...
    if [ $# -eq 1 ]
    then
	java -classpath ${CLASSPATH}:../lib/pinpoint.jar:../ssm.jar:. ${PUBLISHER} ${HOSTNAME} -Xms384M -Xmx384M roc.rr.ssm.Start b $1 > brick$1-log 2>&1
    else
	echo "usage: brick.sh <numerical id of invoking brick>"
    fi
