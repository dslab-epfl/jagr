...
public class DBUtil
{
    ...
    private PreparedStatement addComp;  // add a row to the 'components' table
    private PreparedStatement addFault; // add a row to the 'faults' table

    /* Prepared statements for removing stuff */
    private PreparedStatement remComp;  // removes a row from the 'components' table
    private PreparedStatement remServlet; //removes a row from the 'servlets' table
	...
	try {
		...
		"INSERT INTO faults(Component,Method,Occurred,Description,Source,Notes) VALUES(?,?,?,?,?,?)");
	}
	catch( Exception e ) { 
	    handleException(e); 
	}
     ...
     *
     **/
    synchronized public void storeComponent( Component comp )
	throws SQLException
    {
	addComp.setString( 1, comp.canonicalName );        // UID column
	addComp.setString( 2, comp.shortName );            // Name column
	addComp.setString( 3, comp.type );                 // Type column
	if ( comp.parent != null ) {                       // Parent column
	    addComp.setString( 4, comp.parent.canonicalName );
	} else {
	    addComp.setString( 4, "" );
	}
	...
	try {
	    ...
	    removeComponent( comp.canonicalName );
	    addComp.executeUpdate();
	}
    }
     ...
     *
     **/
    synchronized public void removeComponent( String compName )
	throws SQLException
    {
